...
export async function fetchUser(): Promise<User> {
    try {
        ...
        console.error(err)
        showToast(ToastStyle.Failure, 'Failed to fetch user info')
...
export async function fetchProjects(
    ...
    const projectsWithoutDuplicates = projects.filter((value, index, self) =>
...
async function _rawFetchProjects(
    team?: Team,
    limit = 100
): Promise<Project[]> {
    ...
    try {
        ...
        const response = await fetch(
            apiURL + `v8/projects?${query.toString()}`,
            {
                method: 'get',
                headers: headers,
            }
        ...
        console.error(err)
        showToast(ToastStyle.Failure, 'Failed to fetch projects')
...
export async function deleteProjectById(projectId: string, teamId?: string) {
    try {
        const response = await fetch(
            apiURL + `v8/projects/${projectId}?teamId=${teamId ?? ''}`,
            {
                method: 'delete',
                headers: headers,
            }
        ...
        console.error(e)
        showToast(ToastStyle.Failure, 'Failed to delete project')
...
export async function deleteEnvironmentVariableById(
    ...
    try {
        ...
        console.error(e)
        showToast(ToastStyle.Failure, 'Failed to delete environment variable')
...
export async function fetchProjectById(projectId: string, teamId?: string) {
    try {
        ...
        console.error(err)
        showToast(ToastStyle.Failure, 'Failed to fetch project')
...
async function _rawFetchProjectEnvironmentVariables(
    projectId: string,
    teamId?: string
): Promise<Environment[]> {
    dayjs.extend(relativeTime)
    try {
        const response = await fetch(
            apiURL + `v8/projects/${projectId}/env?teamId=${teamId ?? ''}`,
            {
                method: 'get',
                headers: headers,
            }
